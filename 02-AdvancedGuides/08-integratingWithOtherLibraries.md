...
Next, we will implement the lifecycle methods.  We will to initialize `Chosen` with the ref to the `<select>` node in `componentDidMount`, adn tear it down in `componentWillUnmount`:
